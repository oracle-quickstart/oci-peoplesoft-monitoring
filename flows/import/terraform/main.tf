...
resource "null_resource" "import_lookups" {
  ...
  provisioner "local-exec" {
    command = "python3 ./scripts/import_lookup.py -t Lookup -a ${var.auth_type} -p ${var.config_file_profile} -n \"PSFT Functional Sensors\" -f ${local.kc_path}/logan-lookups/PSFT_Lookup.csv"
  }
}
